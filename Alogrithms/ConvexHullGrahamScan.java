...
public class ConvexHullGrahamScan {
	...
	private void sortByAngle(List<Point> points, Point ref) {
		Collections.sort(points, (b, c) -> {
			 ...
			 */
			if (b.x == ref.x && b.y==ref.y) return -1;
			if (c.x == ref.x && c.y==ref.y) return 1;
	...
	public int ccw(Point a,Point b,Point c){
		return (c.x-b.y)*(b.x-a.x) - (b.y-a.y)*(c.x-b.x);
	}
	public List<Point> scan(List<Point> points) {
		 ...
		 */
		Point minYPoint=new Point(0,Integer.MAX_VALUE);
		for(int i=0;i<points.size();i++){
			if(points.get(i).y<minYPoint.y){
				minYPoint.x=points.get(i).x;
				minYPoint.y=points.get(i).y;
			}
		}
	...
	public static void main(String[] args) {
		...
		List<Point> result=hull.scan(points);
		for(Point p: result){
